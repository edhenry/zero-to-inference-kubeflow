...
def conv_model(features: tf.Tensor, labels: tf.Tensor, mode: str, params: Dict[str, str]) -> tf.estimator.EstimatorSpec:
    ...
    with tf.variable_scope('conv_layer1'):
        h_conv1 = tf.layers.conv2d(feature,
                                   filters=32,
                                   kernel_size=[5, 5],
                                   padding='same',
                                   activation=tf.nn.relu)

        h_pool1 = tf.layers.max_pooling2d(h_conv1, 
                                          pool_size=2, 
                                          strides=2, 
                                          padding='same')
    ...
    with tf.variable_scope('conv_layer2'):
        h_conv2 = tf.layers.conv2d(h_pool1,
                                   filters=64,
                                   kernel_size=[5, 5],
                                   padding='same',
                                   activation=tf.nn.relu)

        h_pool2: tf.layers.max_pooling2d = tf.layers.max_pooling2d(h_conv2, 
                                                                   pool_size=2, 
                                                                   strides=2, 
                                                                   padding='same')
    ...
    h_fc1: tf.layers.dense = tf.layers.dense(h_pool2_flat, 1024, activation=tf.nn.relu)
    h_fc1: tf.layers.dropout = tf.layers.dropout(h_fc1,
                                                 rate=0.5,
                                                 training=(mode == tf.estimator.ModeKeys.TRAIN))

    # Compute logits (1 per class) and compute loss.
    logits: tf.layers.dense = tf.layers.dense(h_fc1, N_DIGITS, activation=None)
    predict: tf.layers.softmax = tf.nn.softmax(logits)
    classes: tf.Tensor = tf.cast(tf.argmax(predict, 1), tf.uint8)

    # Compute predictions.
    predicted_classes: tf.Tensor = tf.argmax(logits, 1)
    ...
    eval_metric_ops: Dict[str, tf.metrics] = {
...
def cnn_serving_input_receiver_fn() -> tf.estimator.export.ServingInputReceiver:
    inputs: Dict[str, tf.Tensor] = {X_FEATURE: tf.placeholder(tf.float32, [None, 28, 28])}
    return tf.estimator.export.ServingInputReceiver(inputs, inputs)


def linear_serving_input_receiver_fn() -> tf.estimator.export.ServingInputReceiver:
    inputs: Dict[str, tf.Tensor] = {X_FEATURE: tf.placeholder(tf.float32, (784,))}
    return tf.estimator.export.ServingInputReceiver(inputs, inputs)
...
def main(_: None) -> None:
    ...
    tf.logging.info("cluster=%s job_name=%s task_index=%s", 
                    cluster, 
                    job_name,
                    task_index)
    ...
    test_input_fn = tf.estimator.inputs.numpy_input_fn(x={X_FEATURE: mnist.train.images},
                                                       y=mnist.train.labels.astype(np.int32),
                                                       num_epochs=1,
                                                       shuffle=False)

    training_config = tf.estimator.RunConfig(model_dir=args.tf_model_dir, 
                                             save_summary_steps=100, 
                                             save_checkpoints_steps=1000)

    if args.tf_model_type == "LINEAR":
        # Linear classifier.
        feature_columns = [tf.feature_column.numeric_column(X_FEATURE, shape=mnist.train.images.shape[1:])]
        ...
        export_final = tf.estimator.FinalExporter(args.tf_export_dir, 
                                                  serving_input_receiver_fn=cnn_serving_input_receiver_fn)
